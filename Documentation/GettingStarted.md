...
The CA.Blocks.DataAccess have been published onto NuGet.  First thing that you need to decide is which provider we want to use. 

For SQL server https://www.nuget.org/packages/CA.Blocks.SQLServerDataAccess/
```
PM> Install-Package CA.Blocks.SQLServerDataAccess -Version 2.2.21
```

For SQL Microsoft.Data.Sqlite https://www.nuget.org/packages/CA.Blocks.SQLLiteDataAccess/
```
PM> Install-Package CA.Blocks.SQLLiteDataAccess -Version 2.2.21
```
...
2. The example above is using the provided HardCodedConnectionStringsResolver, this is provided for quick prototyping, samples and testing code allowing connection to be specified in line with the code. It is recommended you use an external connection string with working on something that is to be published.
3. The ReadSysObjectsOfType method represents your DataAccess Method, the only parameter exposed is xtype
4. The CreateTextCommand will return a Interface to the SQL server implementation of the command
5. The SQL is constructed internally as parameterized query
6. The conversion of .NET string to SQL parameter is done in the cmd.Parameters.Add(xtype.ToSqlParameter("@xtype"));

...
Given the example where we would like to get the the ProductCategory  the ProductSubCategory and the ProductNameAndNumber we could execute this as three methods:
...
If we know we always what to work with all three sets of data we can execute wrap execute up into single ToResultsSet. The key advantage is less wait time as all three sets can be fetched from the database at the same tim.

To do this simply execute the result into ToResultsSet\<T1, T2, ... T5> the blocks has support for up to 5 result sets in a single statement. 
